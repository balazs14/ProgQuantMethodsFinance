...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$ErrorActionPreference='Stop'; $scriptPath='%~f0'; $scriptDir=Split-Path -Parent $scriptPath; $assignment='%ASSIGNMENT%'; $lines=Get-Content -LiteralPath $scriptPath; $guardLine=($lines|Select-String -Pattern '^__EXAM_GUARD_PAYLOAD_BELOW__$'|Select-Object -First 1).LineNumber; $workspaceLine=($lines|Select-String -Pattern '^__WORKSPACE_PAYLOAD_BELOW__$'|Select-Object -First 1).LineNumber; if (-not $guardLine -or -not $workspaceLine) {throw 'Workspace data missing'}; $temp=Join-Path $env:TEMP ('pqmf_'+[guid]::NewGuid().ToString('N')); New-Item -ItemType Directory -Path $temp|Out-Null; [IO.File]::WriteAllBytes((Join-Path $temp 'payload.zip'),[Convert]::FromBase64String((-join $lines[$workspaceLine..($lines.Count-1)]))); if ($workspaceLine -gt ($guardLine+1)) {[IO.File]::WriteAllBytes((Join-Path $temp 'guard.vsix'),[Convert]::FromBase64String((-join $lines[$guardLine..($workspaceLine-2)])))}; $extractRoot=Join-Path $scriptDir ($assignment+'_OPEN'); New-Item -ItemType Directory -Path $extractRoot -Force|Out-Null; $existing=Get-ChildItem -LiteralPath $extractRoot -Directory -Recurse -Force -ErrorAction SilentlyContinue|Where-Object {$_.Name -eq '.vscode'}|Select-Object -First 1; $preserved=Join-Path $temp 'preserved-content'; if ($existing) {$project=$existing.Parent.FullName; New-Item -ItemType Directory -Path $preserved -Force|Out-Null; $files=@(Get-ChildItem -LiteralPath $project -File -Recurse -Force|Where-Object {$_.FullName -notlike (Join-Path $project '.vscode\*') -and $_.FullName -notlike (Join-Path $project '.venv\*') -and $_.FullName -notlike (Join-Path $project 'venv\*') -and $_.FullName -notlike (Join-Path $project '.pqmf-guard-quarantine\*') -and $_.Name -notlike '.pqmf-exam-guard-*'}); foreach ($file in $files) {$relative=$file.FullName.Substring($project.Length).TrimStart([char]92,[char]47); $destination=Join-Path $preserved $relative; New-Item -ItemType Directory -Path (Split-Path -Parent $destination) -Force|Out-Null; Copy-Item -LiteralPath $file.FullName -Destination $destination -Force}; Remove-Item -LiteralPath $project -Recurse -Force}; Expand-Archive -LiteralPath (Join-Path $temp 'payload.zip') -DestinationPath $extractRoot -Force; $project=(Get-ChildItem -LiteralPath $extractRoot -Directory -Recurse -Force|Where-Object {$_.Name -eq '.vscode'}|Select-Object -First 1).Parent.FullName; if (Test-Path $preserved) {Get-ChildItem -LiteralPath $preserved -Force|Copy-Item -Destination $project -Recurse -Force}; $python=Get-Command python -ErrorAction SilentlyContinue; if (-not $python) {$python=Get-Command python3 -ErrorAction SilentlyContinue}; if ($python) {Get-ChildItem -LiteralPath $project -Filter *.ipynb -File -Recurse|ForEach-Object {& $python.Source -c \"import hashlib,json,sys; p=sys.argv[1]; d=json.load(open(p)); d.setdefault('metadata',dict())['vscode']={'interpreter':{'hash':hashlib.sha256(sys.argv[2].encode()).hexdigest()}}; json.dump(d,open(p,'w'),indent=1)\" $_.FullName $python.Source}}; $settings=Join-Path $project '.vscode\settings.json'; if (Test-Path $settings -and $python) {$d=Get-Content $settings -Raw|ConvertFrom-Json; $d|Add-Member -NotePropertyName 'python.defaultInterpreterPath' -NotePropertyValue $python.Source -Force; $d|ConvertTo-Json -Depth 20|Set-Content $settings}; if (Test-Path (Join-Path $temp 'guard.vsix')) {$code=Get-Command code -ErrorAction SilentlyContinue; if (-not $code) {$locations=@((Join-Path $env:LOCALAPPDATA 'Programs\Microsoft VS Code\Code.exe'),(Join-Path $env:ProgramFiles 'Microsoft VS Code\Code.exe')); $code=$locations|Where-Object {Test-Path $_}|Select-Object -First 1}; & $code.Source --install-extension (Join-Path $temp 'guard.vsix') --force|Out-Null; $vscode=Join-Path $project '.vscode'; Remove-Item (Join-Path $vscode '.pqmf-exam-guard-extension-heartbeat'),(Join-Path $vscode '.pqmf-exam-guard-script-heartbeat'),(Join-Path $vscode '.extensions-check.lock') -Recurse -Force -ErrorAction SilentlyContinue; Get-ChildItem -LiteralPath $vscode -File|Where-Object {$_.Name -ne 'settings.json'}|ForEach-Object {$_.IsReadOnly=$true}}; Remove-Item $temp -Recurse -Force; Start-Process -FilePath $code.Source -ArgumentList @('--disable-workspace-trust','--reuse-window',$project)"
if errorlevel 1 exit /b 1
endlocal
exit /b 0
