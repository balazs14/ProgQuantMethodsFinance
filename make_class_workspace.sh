...
:; if [ -z "${PQMF_RELAUNCHED:-}" ] && [ "$(uname)" = "Darwin" ]; then SELF_DIR="$(cd "$(dirname "$0")" && pwd)"; SELF_BASE="$(basename "$0")"; RELAUNCH_CMD="cd $(printf %q "$SELF_DIR") && PQMF_RELAUNCHED=1 bash $(printf %q "$SELF_BASE")"; if osascript -e 'on run argv' -e 'tell application "Terminal" to activate' -e 'tell application "Terminal" to do script (item 1 of argv)' -e 'end run' "$RELAUNCH_CMD" >/dev/null 2>&1; then echo "Opening a new Terminal window to set up your workspace..."; exit 0; fi; fi
...
:; if [ -z "$CODE_CMD" ]; then echo "VS Code was not found on PATH; the workspace will still be set up and its path printed at the end."; fi
:; PYTHON_CMD=""
...
:; IS_EXAM=0; if [ -s "$WORKDIR/exam-guard.b64" ]; then IS_EXAM=1; fi
...
:; if [ -s "$WORKDIR/exam-guard.b64" ]; then EXISTING_VSCODE=$(find "$EXTRACT_DIR" -type d -name ".vscode" -print -quit); if [ -n "$EXISTING_VSCODE" ]; then SETUP_PROJECT=$(dirname "$EXISTING_VSCODE"); CONTENT_ACTION="keep"; mkdir -p "$WORKDIR/preserved-content"; find "$SETUP_PROJECT" \( -path "$SETUP_PROJECT/.vscode" -o -path "$SETUP_PROJECT/.venv" -o -path "$SETUP_PROJECT/venv" -o -path "$SETUP_PROJECT/.pqmf-guard-quarantine" -o -path "$SETUP_PROJECT/.pqmf-exam-guard-preserved-content" \) -prune -o -type f ! -name ".pqmf-exam-guard-setup" ! -name ".pqmf-exam-guard-preserved-log" ! -name ".pqmf-exam-guard-preserved-violation" ! -name ".pqmf-exam-guard-content-choice" -print0 | while IFS= read -r -d '' CONTENT_FILE; do CONTENT_REL=${CONTENT_FILE#"$SETUP_PROJECT/"}; mkdir -p "$WORKDIR/preserved-content/$(dirname "$CONTENT_REL")"; cp "$CONTENT_FILE" "$WORKDIR/preserved-content/$CONTENT_REL"; done; rm -rf "$SETUP_PROJECT"; fi; fi
...
:; if [ "$IS_EXAM" = "1" ] && [ -n "$CODE_CMD" ]; then date +%s > "$PROJECT_DIR/.pqmf-exam-guard-setup"; fi
:; rm -rf "$WORKDIR"
:; OPENED=0
:; if [ -n "$CODE_CMD" ]; then echo "Opening: $PROJECT_DIR"; if "$CODE_CMD" --disable-workspace-trust --reuse-window "$PROJECT_DIR"; then OPENED=1; fi; fi
:; if [ "$OPENED" = "1" ]; then if [ "$(uname)" = "Darwin" ]; then osascript -e "tell application \"Terminal\" to close (every window whose selected tab's tty is \"$(tty)\")" >/dev/null 2>&1 || true; fi; else open "$PROJECT_DIR" >/dev/null 2>&1 || true; echo ""; echo "=================================================================="; echo "Could not open VS Code automatically."; echo "Your workspace has been prepared here:"; echo "  $PROJECT_DIR"; if [ "$IS_EXAM" = "1" ]; then echo "This is an exam workspace. Install VS Code from https://code.visualstudio.com/ then run this launcher again so the exam guard is set up."; else echo "Install VS Code from https://code.visualstudio.com/ (or open it if already installed),"; echo "then choose File > Open Folder... and select the folder shown above."; fi; echo "=================================================================="; fi
:; exit 0
...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$vc=Join-Path $env:SystemRoot 'System32\vcruntime140.dll'; if (-not (Test-Path -LiteralPath $vc)) {$arch=$env:PROCESSOR_ARCHITECTURE; if ($arch -eq 'ARM64') {$url='https://aka.ms/vs/17/release/vc_redist.arm64.exe'} else {$url='https://aka.ms/vs/17/release/vc_redist.x64.exe'}; $msg='Microsoft Visual C++ Redistributable is not installed. VS Code needs it to detect Python and run notebooks. Install it from ' + $url + ' then run this launcher again.'; try {Add-Type -AssemblyName PresentationFramework; [System.Windows.MessageBox]::Show($msg,'PQMF - missing prerequisite',[System.Windows.MessageBoxButton]::OK,[System.Windows.MessageBoxImage]::Warning) | Out-Null} catch {Write-Host $msg}}"
...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$ErrorActionPreference='Stop'; if ('%ASSIGNMENT%' -like '*_exam') {$extractRoot=Join-Path (Split-Path -Parent '%~f0') ('%ASSIGNMENT%_OPEN'); $existingVscode=Get-ChildItem -LiteralPath $extractRoot -Directory -Recurse -Force -ErrorAction SilentlyContinue|Where-Object {$_.Name -eq '.vscode'}|Select-Object -First 1; if ($existingVscode) {$project=$existingVscode.Parent.FullName; $preservedContent=Join-Path $project '.pqmf-exam-guard-preserved-content'; Remove-Item -LiteralPath $preservedContent -Recurse -Force -ErrorAction SilentlyContinue; New-Item -ItemType Directory -Path $preservedContent -Force|Out-Null; $contentFiles=@(Get-ChildItem -LiteralPath $project -File -Recurse -Force|Where-Object {$_.FullName -notlike (Join-Path $project '.vscode\*') -and $_.FullName -notlike (Join-Path $project '.venv\*') -and $_.FullName -notlike (Join-Path $project 'venv\*') -and $_.FullName -notlike (Join-Path $project '.pqmf-guard-quarantine\*') -and $_.FullName -notlike (Join-Path $project '.pqmf-exam-guard-preserved-content\*') -and $_.Name -notin @('.pqmf-exam-guard-setup','.pqmf-exam-guard-preserved-log','.pqmf-exam-guard-preserved-violation','.pqmf-exam-guard-content-choice')}); foreach ($file in $contentFiles) {$relative=$file.FullName.Substring($project.Length).TrimStart([char]92,[char]47); $destination=Join-Path $preservedContent $relative; New-Item -ItemType Directory -Path (Split-Path -Parent $destination) -Force|Out-Null; Copy-Item -LiteralPath $file.FullName -Destination $destination -Force}; Remove-Item -LiteralPath $project -Recurse -Force}}; exit 0"
...
for notebook_path in pathlib.Path(sys.argv[1]).rglob("*.ipynb"):
    ...
    with notebook_path.open("w", encoding="utf-8") as notebook_file:
        json.dump(notebook, notebook_file, ensure_ascii=True, indent=1)
        notebook_file.write("\n")
PY
    # Windows PowerShell 5.1 mis-reads non-ASCII in a BOM-less file (as ANSI), turning
    # bytes into "smart quotes" that corrupt tokenizing and break here-strings. So force
    # the Windows-executed .ps1 scripts to pure ASCII (mapping common glyphs) + CRLF.
    # Done before the VSIX/manifest build and the zip so the manifest hashes match.
    for ps1 in "$foldername/.vscode/extensions.ps1" "$foldername/.vscode/terminal-rc.ps1"; do
        [ -f "$ps1" ] && python3 -c "import sys; p=sys.argv[1]; s=open(p,encoding='utf-8').read(); s=s.replace(chr(0x2713),'[OK]').replace(chr(0x2717),'[X]').replace(chr(0x26a0)+chr(0xfe0f),'[!]').replace(chr(0x26a0),'[!]').replace(chr(0x2026),'...'); s=''.join(c if ord(c)<128 else '?' for c in s); s=s.replace('\r\n','\n').replace('\n','\r\n'); open(p,'w',encoding='ascii',newline='').write(s)" "$ps1"
    done
...
make_workspace "00_CourseLevelDocuments/hello_world.*" ALL ALL exam "" "{\"pqmfExamGuard.disableOutOfFocusCheck\": false, \"pqmfExamGuard.disableOutsideWorkspaceCheck\": false}"

make_workspace "../ProgramozasAlapjai/02_NumbersTypesInputConditionals/NumbersTypesInputConditionals_class.ipynb" W02 GALL class "HU" "{}"
make_workspace "03_SequenceTypesListsLoops/SequenceTypesListsLoops_class.ipynb" C03 GALL class "" "{}"
