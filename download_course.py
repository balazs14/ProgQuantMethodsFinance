...
ROOT_URL = "https://corvinus-materials.azurewebsites.net/python_psz/"
OUTPUT_ROOT = Path(__file__).resolve().parent / "course_mirror"
...
def sanitize_segment(segment):
    ascii_segment = unicodedata.normalize("NFKD", segment).encode("ascii", "ignore").decode("ascii")
...
def sanitize_path(path):
    parts = [sanitize_segment(part) for part in Path(path).parts if part not in ("/", "")]
...
def rewrite_html_links(text, page_url, target_path):
    def replace_link(match):
        ...
        if not (should_queue_html(absolute) or should_download_asset(absolute)):
            return match.group(0)
        relative = os.path.relpath(local_path_for(absolute), start=target_path.parent).replace(os.sep, "/")
...
def write_directory_indexes():
    ...
    for directory in directories:
                ...
                if directory != OUTPUT_ROOT:
                        parent_relative = os.path.relpath(directory.parent / "index.html", start=directory).replace(os.sep, "/")
                        entries.append(f'    <li><a href="{parent_relative}">..</a></li>')

                for child in sorted(directory.iterdir(), key=lambda item: (not item.is_dir(), item.name.lower())):
                        if child.name == ".DS_Store" or child.name == "index.html":
                                continue
                        if child.is_dir():
                ...
                title = f"Index of /{directory.relative_to(OUTPUT_ROOT).as_posix()}" if directory != OUTPUT_ROOT else "Index of /"
                html = """<!doctype html>
<html lang="en">
<head>
    <meta charset="utf-8">
...
def main():
    ...
    top_level = sorted({Path(path).parts[0] for path in downloaded if Path(path).parts})
